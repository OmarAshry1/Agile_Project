...
/**
 * Controller for Student My Enrolled Courses
...
public class StudentMyCoursesController {
    ...
    private boolean checkStudentAccess() {
        if (!authService.isLoggedIn()) {
            showError("Access Denied", "Please login to view your courses.");
            return false;
        }
    ...
    private void loadEnrollments() {
        try {
            ...
            List<Enrollment> enrollments = enrollmentService.getStudentEnrollments(student, true); // Enrolled only
            ...
            totalCreditsLabel.setText("Total Credits: " + totalCredits + " / 18");
            enrolledCoursesLabel.setText("Enrolled Courses: " + enrollments.size());
            
            statusLabel.setText("Loaded " + enrollments.size() + " enrollment(s)");
    ...
    private void handleDrop() {
        ...
        if (selectedEnrollment.getStatus() != EnrollmentStatus.ENROLLED) {
            showWarning("Invalid Status", "You can only drop courses with ENROLLED status.");
            return;
        }
    ...
    private void disableAllControls() {
        ...
        if (dropButton != null) dropButton.setDisable(true);
    }
