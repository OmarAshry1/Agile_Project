...
/**
 * Controller for Edit Course Form
 */
public class EditCourseController {
    ...
    private void loadCourseData() {
        ...
        courseInfoLabel.setText("Editing: " + course.getCode() + " - " + course.getName());
        ...
        typeComboBox.setValue(course.getType().toString());
        maxSeatsField.setText(String.valueOf(course.getMaxSeats()));
    }
    ...
    private void handleSave() {
        ...
        try {
            ...
            boolean updated = courseService.updateCourse(
                course.getId(), code, name, description, credits, department, semester, type, maxSeats
            );
            ...
            if (e.getMessage().contains("UNIQUE") || e.getMessage().contains("duplicate")) {
                ...
                showError("Database Error", "Failed to update course: " + e.getMessage());
            }
        } catch (Exception e) {
    ...
    private boolean checkAdminAccess() {
        if (!authService.isLoggedIn() || !"ADMIN".equals(authService.getCurrentUserType())) {
            showError("Access Denied", "Only administrators can edit courses.");
            return false;
        }
        return true;
    }
